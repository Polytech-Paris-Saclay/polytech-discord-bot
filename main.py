...
async def tesla():
    ...
    previousListings = [int(message.embeds[0].footer.text) for message in messages]
...
async def grades():
    ...
    messages = await channel.history(limit=None).flatten()
    previousGrades = [message.embeds[0].footer.text for message in messages]
    newGrades = [
    ...
    for grade in newGrades:
        ...
        embed.set_thumbnail(bot.user.avatar)
